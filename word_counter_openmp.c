...
// // Global storage to avoid large stack allocations
// char allWords[MAX_WORDS][MAX_WORD_LEN];
// WordList threadWordLists[NUM_THREADS];
// WordList globalWordList = {.count = 0};
...
// // Add or update word in a given WordList
...
//         list->count++;
//     } else {
//         fprintf(stderr, "Exceeded max unique words in a thread!\n");
//         exit(1);
//     }
// }

// // Merge a thread-local WordList into the global one
// void mergeWordLists(WordList* dest, WordList* src) {
//     for (int i = 0; i < src->count; i++) {
//         addWordToList(dest, src->words[i].word);
//         for (int j = 0; j < dest->count; j++) {
//             if (strcmp(dest->words[j].word, src->words[i].word) == 0) {
//                 dest->words[j].count += src->words[i].count - 1;
//                 break;
//             }
//         }
//     }
...
//     // Read and clean words into from the input text file
...
//             } else {
//                 fprintf(stderr, "Exceeded max words limit!\n");
//                 fclose(file);
//                 return 1;
//             }
//         }
...
//     // Step 3: Merge all thread-local results into global list
